...
upper_lag=, slstay=0.10, logodds=0, select=, dseff=);
** to compare OLS vs all AR models with lag =1, 2, ..., upper_lag,; 
**    vs backward AR;
**    in terms of residual plots, fit plots, and fit summary;
**    also: OLS with Newey-West corrected std.errors is conducted;
...
%IF &logodds=1 %THEN 
	...
	footnote1 h=0.8 "0% is set to 0.0001% -> logodds=-13.8, 100% is set to 99.9999% -> logodds=+13.8";
	%END;
%ELSE %DO;
...
data _fitsummary; set _fitsummary;
 ...
 data _solutions; 
	...
	%DO;outcome="log odds &outcome";outcome_label="log odds &outcome_label";%END;
	model="backw AR";
	model_descrip="ITS: &period &trt postperiod*&trt // backwards AR from &max_backwards";
...
data _solutions; 
	...
	select=0; %IF &select=nw %THEN %DO; select=1;%END;
	rename variable=label;
	keep &cluster outcome outcome_label variable estimate lower upper stderr probt 
		model model_descrip select;
	run;
...
upper_lag=, slstay=0.10,select=, dsdescrip=,dseff=);
	* descriptive;
	...
	* inferential;
	...
	upper_lag=&upper_lag, slstay=&slstay,logodds=0, select=&select, dseff=&dseff);
%mend its_cont;
...
upper_lag=, slstay=0.10,select=, dsdescrip=,dseff=);
	...
	* inferential;
	%explore_ar(ds=&ds,outcome=&outcome, cluster=&cluster,period=&period,trt=&trt,
	upper_lag=&upper_lag, slstay=&slstay,logodds=&logodds, select=&select, dseff=&dseff);
%mend its_bin;
